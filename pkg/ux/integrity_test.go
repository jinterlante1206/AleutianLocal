...
// =============================================================================
// QuickChainVerifier Tests
// =============================================================================

func TestQuickChainVerifier_Verify_EmptyChain(t *testing.T) {
...
func TestQuickChainVerifier_Verify_SingleEvent(t *testing.T) {
	verifier := NewQuickChainVerifier()
	events := []StreamEvent{
		{Hash: "abc123", PrevHash: ""},
	}

	result := verifier.Verify(events)

	if !result.Valid {
		t.Error("single event with empty PrevHash should be valid")
	}
	if result.ChainLength != 1 {
		t.Errorf("ChainLength = %d, want 1", result.ChainLength)
	}
	if result.FinalHash != "abc123" {
		t.Errorf("FinalHash = %q, want %q", result.FinalHash, "abc123")
	}
}

func TestQuickChainVerifier_Verify_ValidChain(t *testing.T) {
	verifier := NewQuickChainVerifier()
	events := []StreamEvent{
		{Hash: "hash1", PrevHash: ""},
	...
	if !result.Valid {
		t.Errorf("valid chain should pass verification: %s", result.ErrorMessage)
	}
	if result.ChainLength != 3 {
		t.Errorf("ChainLength = %d, want 3", result.ChainLength)
	}
	if result.FinalHash != "hash3" {
		t.Errorf("FinalHash = %q, want %q", result.FinalHash, "hash3")
	}
	if result.InvalidEventIndex != -1 {
		t.Errorf("InvalidEventIndex = %d, want -1", result.InvalidEventIndex)
	}
...
func TestQuickChainVerifier_Verify_FirstEventWithPrevHash(t *testing.T) {
	verifier := NewQuickChainVerifier()
	events := []StreamEvent{
		{Hash: "hash1", PrevHash: "unexpected"},
	}

	result := verifier.Verify(events)

	if result.Valid {
		t.Error("first event with non-empty PrevHash should fail")
	}
	...
	if result.ActualHash != "unexpected" {
		t.Errorf("ActualHash = %q, want %q", result.ActualHash, "unexpected")
	}
}

func TestQuickChainVerifier_Verify_BrokenChain(t *testing.T) {
	verifier := NewQuickChainVerifier()
	events := []StreamEvent{
		{Hash: "hash1", PrevHash: ""},
		{Hash: "hash2", PrevHash: "hash1"},
		{Hash: "hash3", PrevHash: "wrong"}, // Should be "hash2"
	}
	...
	if result.InvalidEventIndex != 2 {
		t.Errorf("InvalidEventIndex = %d, want 2", result.InvalidEventIndex)
	}
	if result.ExpectedHash != "hash2" {
		t.Errorf("ExpectedHash = %q, want %q", result.ExpectedHash, "hash2")
	}
	if result.ActualHash != "wrong" {
		t.Errorf("ActualHash = %q, want %q", result.ActualHash, "wrong")
	}
